...
class BeerSong
  ...
  def print_song
    ...
    @num.downto(1) {|n|
    puts "#{translate(n).capitalize} #{bottle n} of beer on the wall,"        ,
         "#{translate(n).capitalize} #{bottle n} of beer,"                    ,
         "Take one down, pass it around,"                         ,
         "#{translate(n-1).capitalize} #{bottle n-1} of beer on the wall."}
  end
